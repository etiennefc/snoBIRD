...
all_preds_paths = str(sys.argv[1])
all_preds = []
with open(all_preds_paths, 'r') as file_:
    for line in file_:
        all_preds.append(line.strip('\n'))
...
chr_dictio = {record.id: len(str(record.seq)) 
                for record in SeqIO.parse(input_fasta, "fasta")}


# Deal with input that are the size or smaller than window_size (return that 
# window only with filter on probability)
if len([v for k,v in chr_dictio.items() if v > fixed_length]) > 0:  # input bigger than fixed_length
...
else:  # if all input sequences <= 194 nt
    # Concat all predictions of first SnoBIRD model into 1 df (preds from all 
    # input sequences)
    #Raise warning as to updated prob_threshold
    if prob_threshold == 0.999: # default value
        # define less stringent threshold because there are fewer sequences to 
        # predict on compared to the whole genome
        prob_threshold = 0.9
        import warnings
        warnings.filterwarnings("always")
        warnings.formatwarning = lambda msg, *args: f"\033[93m{msg}\033[0m\n"  
        warnings.warn("\nUserWarning: Default probability threshold for "+
        "SnoBIRD's first model was lowered to 0.9 as all your input sequences"+
        " are of length <= 194 nt (i.e. less stringent than a whole-genome "+
        "prediction).")
        warnings.filterwarnings("ignore")
    ...
    # Readjust columns
    df_preds['name'] = df_preds['chr']
    df_preds['len'] = df_preds['end'] - df_preds['start']
    df_preds = df_preds.rename(columns={'chr': 'chrom', 'probability_first_model': 'score'})
    df_preds[['chrom', 'start', 'end', 'name', 'score', 'strand', 'len']].reset_index(
    drop=True).to_csv(output_df, sep='\t', index=False, header=False)

    # Redundant save because the center window is the same size as the inital window
    df_preds = df_preds.reset_index(drop=True)
    df_preds['index_'] = df_preds.index + 1
    df_preds['prediction_id'] = 'CD_' + df_preds.index_.astype(str)
    df_preds = df_preds.rename(columns={'start': 'centered_start', 'end': 'centered_end'})
    ...
    with open(input_fasta, 'r') as f:
        for line in f:
            if line.startswith('>'):
                ...
                seq_ = line.strip('\n')
                dictio[id_] = seq_
    df_preds[f'extended_{fixed_length}nt_sequence'] = df_preds['name'].map(dictio)

    df_preds[cols_bed + [f'extended_{fixed_length}nt_sequence']].to_csv(
                        output_df_center, sep='\t', index=False, header=False)
