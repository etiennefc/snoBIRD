...
X = X.filter(regex='^[^N_]')  # remove "N_" features
...
for fold_index, (train_index, test_index) in enumerate(skf.split(x_tensor.numpy(), y_tensor.numpy())):
    ...
    output_path = [path for path in output_figure if f'_fold_{fold_i}_ATCG.svg' in path][0]
    ...
    torch.save(model.state_dict(), [p for p in output_model if f'fold_{fold_i}_ATCG.pt' in p][0])
