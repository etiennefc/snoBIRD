...
one_hot_output = [path for path in snakemake.output 
                    if 'one_hot' in path]
scaled_encoded_output = [path for path in snakemake.output 
                            if 'scaled' in path]
...
for i, df in enumerate([tuning, training, test]):
    ...
    # Scale/normalize the features using standardization
    df_num = df.copy()
    ...
    scaled_df = scaled_df.filter(regex='_norm$|gene_id')
    scaled_df.to_csv(scaled_encoded_output[i], index=False, sep='\t')  # save one-hot-encoded scaled output
