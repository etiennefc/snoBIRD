...
rfam_pseudo = rfam_pseudo[['gene_id', 'chr', 'strand', 'start', 'end', 'sequence', 
                        'extended_sequence', 'rfam_family_id', 'species_name', 
                        f'extended_{fixed_length}nt_sequence', 'gene_biotype']]
...
sno_species_df['species_name'] = sno_species_df['species_name'].str.replace('drosophila_', 'D_')
...
sno_rfam['gene_biotype'] = 'expressed_CD_snoRNA'

# Concat expressed sno and pseudosno
sno_rfam = pd.concat([sno_rfam, rfam_pseudo])
sno_rfam['species_name'] = sno_rfam['species_name'].str.replace('mus_', 'M_')
sno_rfam['species_name'] = sno_rfam['species_name'].str.replace('homo_', 'H_')
sno_rfam['species_name'] = sno_rfam['species_name'].str.replace('saccharomyces_', 'S_')
sno_rfam['species_name'] = sno_rfam['species_name'].str.replace('drosophila_', 'D_')



print(f'Initial nb of snoRNAs (expressed and pseudogenes): {len(sno_rfam)}')
# Keep only 1 copy per snoRNA duplicates with exactly the same extended se_sno_pseudo_fixed_lengthquences
...
# For the snoRNAs with a Rfam id, separate between those with 1 vs 2 to 100 members vs >100 members per family
...
for i, group in enumerate(sno_w_rfam.groupby('rfam_family_id')):
    ...
    elif len(group[1]) > 100:  # big families
        sno_big_families[group[0]] = group[1]
    else: # 2 to 100 members per family
...
print(sum([len(i) for id_, i in sno_families.items()]), '2_100_members')
print(sum([len(i) for id_, i in sno_big_families.items()]), '>100 members')


# For snoRNAs with > 100 members per family, reduce the number to 100 snoRNAs to avoid overrepresentation of some families
# (maximize the number of different species snoRNAs then randomly choose for the remaining snoRNAs)
def largest_factor(num):
    """ Returns the maximal number if times that num is contained within 100."""
    for i in range(100, 0, -1):  # go backward from num-1 to 1
        if i * num <= 100:
            return i * num
...
for id, df in sno_big_families.items():
    ...
    remaining = 100 - largest  # remaining number of sno that will be randomly chosen within the rest
    ...
    sno_rows_df_list = [pd.DataFrame((sno_rows), 
                        columns=['gene_id', 'chr', 'strand', 'start', 'end', 
                                ...
                                'species_name', f'extended_{fixed_length}nt_sequence', 'gene_biotype'])]
    # Complete with randomly chosen snoRNAs if 100 snoRNAs are not already chosen per family
    if remaining > 0:
...
print(f'After reducing the >100 members to 100: {sum([len(df) for df in big_fam_final_df])} instead of {sum([len(i) for id_, i in sno_big_families.items()])}')
print(f'So total of 2_100 members = {sum([len(df) for df in big_fam_final_df]) + sum([len(i) for id_, i in sno_families.items()])}', 
        f' ({sum([len(i) for id_, i in sno_families.items()])}+{sum([len(df) for df in big_fam_final_df])})')
print(f'So total of 1_100 members = {sum([len(df) for df in big_fam_final_df]) + sum([len(i) for id_, i in sno_families.items()]) + sum([len(i) for id_, i in sno_families_1_member.items()])}', 
...
df0 = pd.concat([df for df in big_fam_final_df])
df1 = pd.concat([i for id_, i in sno_families.items()])
df2 = pd.concat([i for id_, i in sno_families_1_member.items()])
dff = pd.concat([df0, df1, df2, sno_rfam[sno_rfam["rfam_family_id"].isna()]])
print(f'TOTAL OF C/D TO DISPATCH: {s1 + len(sno_rfam[sno_rfam["rfam_family_id"].isna()])} ({s1} + {len(sno_rfam[sno_rfam["rfam_family_id"].isna()])}), i.e. '+\
    f'{len(dff[dff["gene_biotype"] == "expressed_CD_snoRNA"])} expressed C/D and {len(dff[dff["gene_biotype"] == "snoRNA_pseudogene"])} C/D pseudogenes\n')


# Merge the rfam clan id to the 1 and 2-100 snoRNA families df
# Concat the dfs 
...
print(f'Nb of C/D with Rfam Clan id in the 1_member and 2_100_members: {len(sno_fam[~sno_fam["rfam_clan_id"].isna()])}')
...
sno_w_clan = shuffle(sno_fam[~sno_fam['rfam_clan_id'].isna()], random_state=seed)  # length = 567
big_clans_tuning, big_clans_test = [], []
used_clan_id = []
print(f'Based on :\n{coll.Counter(sno_w_clan["rfam_clan_id"])}')
tuning_occurences_per_clan, test_occurences_per_clan = [1, 2, 3, 4, 4, 5, 7, 8, 11, 12], [2, 3, 3, 4, 5, 7, 8, 9, 10, 10, 11, 12, 14, 15]
...
# The sum of occurences (57 and 113 for tuning and test) was chosen 
# pseudo-randomly to make sure that we have ~ 10% (57/567) and 20% (113/567) of sno in clans 
...
sno_no_or_1_rfam_rest, sno_no_or_1_rfam_tuning = train_test_split(sno_no_or_1_rfam, train_size=0.9,  
                                                test_size=0.1, random_state=seed+3)
...
sno_no_or_1_rfam_train, sno_no_or_1_rfam_test = train_test_split(sno_no_or_1_rfam_rest, train_size=0.78, # 78% of the remaining 90% gives ~ 70%
                                                test_size=0.22, random_state=seed+3) # 22% of the remaining 90% gives ~ 20%
...
# For snoRNAs with Rfam id and between 2 to 100 members per family (which are not in a rfam clan previously dispatched)
...
print(f'Remaining C/D to dispatch, i.e. 2_100_members (without Clan id):\n',
        f'{remaining_sno_nb} (Total 1_100_members - CD_w_clan - 1_member_w_family_wo_clan)')

# Count the remaining number of sno needed in each set
tuning_nb_remaining = round(0.1 * remaining_sno_nb)  # 54 C/D
train_nb_remaining = round(0.7 * remaining_sno_nb)  # 377 C/D
test_nb_remaining = round(0.2 * remaining_sno_nb)  # 108 C/D
...
# To get to the 54 remaining C/D to add in the tuning set,
# randomly choose a family of n, n, n, ... snoRNAs (54 snoRNAs)
# This combination of n, n, n, ... was manually picked to ensure a total of 54
...
tun_fam_nb = [11, 10, 7, 5, 4, 3, 2]
tuning_occurences = [1, 1, 1, 1, 2, 3, 2]
tuning_ids = []
...
tuning_df = filtered_df[filtered_df['rfam_family_id'].isin(tuning)]  # 54 C/D

# For test set, randomly choose a family of n, n, n, n, ... snoRNAs (108 snoRNAs)
# This combination of n, n, n, n, ... was manually picked to ensure a total of 108
test_fam_nb = [13, 9, 8, 7, 6, 5, 4, 3, 2]
test_occurences = [1, 1, 2, 1, 2, 3, 2, 6, 5]
...
test_df = filtered_df[filtered_df['rfam_family_id'].isin(test)]  # 108 C/D
...
train_df = filtered_df[~filtered_df['rfam_family_id'].isin(
                        test+tuning+rfam_ids_chosen_clans+list(sno_no_or_1_rfam_tuning.rfam_family_id))]  # 377 C/D


print('Families with the following number of members were chosen randomly for the')
print(f'-Tuning: {tun_fam_nb} (* n fam) --> sum={len(tuning_df)}')
print(f'-Training: the rest --> sum={len(train_df)}')
print(f'-Test: {test_fam_nb} (* n fam) --> sum={len(test_df)}')

# Concat the sets composed of families of 2-100 members to their respective set 
