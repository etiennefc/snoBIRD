...
lr_min = snakemake.params.low_lr
lr_max = snakemake.params.high_lr
...
for fold_index, (train_index, test_index) in enumerate(skf.split(x_tensor.numpy(), y_tensor.numpy())):
    ...
    if optimizer_name == "Adam":
        optimizer = optim.Adam(model.parameters(), lr=lr_min, weight_decay=0.1)
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=lr_min, max_lr=lr_max, 
                    mode='triangular', cycle_momentum=False, step_size_up=1000, step_size_down=1000)
    elif optimizer_name == "AdamW":
        optimizer = optim.AdamW(model.parameters(), lr=lr_min, weight_decay=0.1)
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=lr_min, max_lr=lr_max, 
                    mode='triangular', cycle_momentum=False, step_size_up=1000, step_size_down=1000)
    else:
        optimizer = optim.SGD(model.parameters(), lr=lr_min, weight_decay=0.1)
        scheduler = lr_scheduler.CyclicLR(optimizer, base_lr=lr_min, max_lr=lr_max, 
                    mode='triangular', step_size_up=1000, step_size_down=1000)
    ...
    # Get the f1-score before training (i.e. at epoch 0)
    epoch_f_scores = []
    ...
    temp_df = pd.DataFrame({'epoch': [i for i in range(0, num_epochs+1)], 
