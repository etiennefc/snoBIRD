...
chr_size = snakemake.input.droso_chr_size[0] 
genome = snakemake.input.droso_genome
fixed_length = int(snakemake.wildcards.fixed_length)
pseudogene_df = pd.read_csv(snakemake.input.droso_snoRNA_pseudogenes[0], sep='\t')
...
pseudogene_df_copy.to_csv('droso_temp_pseudogene.bed', sep='\t', index=False, header=False)
sp.call('sort -k1,1 -k2,2n droso_temp_pseudogene.bed > droso_temp_pseudogene_sorted.bed', shell=True)

# Load pseudogene bed and get extended sequence 
pseudogene_bed = BedTool('droso_temp_pseudogene_sorted.bed')
...
pseudogene_df['species_name'] = 'drosophila_melanogaster'
...
sp.call('rm droso_temp_pseudogene*', shell=True)
