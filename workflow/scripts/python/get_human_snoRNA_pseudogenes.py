...
chr_size = snakemake.input.human_chr_size[0] 
genome = snakemake.input.human_genome
fixed_length = int(snakemake.wildcards.fixed_length)
pseudogene_df = pd.read_csv(snakemake.params.human_snoRNA_pseudogenes, sep='\t')
...
pseudogene_df_copy.to_csv('temp_pseudogene.bed', sep='\t', index=False, header=False)
sp.call('sort -k1,1 -k2,2n temp_pseudogene.bed > temp_pseudogene_sorted.bed', shell=True)

# Load pseudogene bed and get extended sequence 211 nt
pseudogene_bed = BedTool('temp_pseudogene_sorted.bed')
...
pseudogene_df['species_name'] = 'homo_sapiens'
...
sp.call('rm temp_pseudogene*', shell=True)
